...
class UpdateCompare {
   ...
   */
  public function update_process_project_info(&$projects): void {
  ...
  function update_calculate_project_update_status(&$project_data, $available): void {
    ...
    try {
      ...
      $project_data['status'] = UpdateFetcherInterface::UNKNOWN;
      $project_data['reason'] = sprintf('Invalid version: %s', $project_data['existing_version']);
      return;
    }
    $supported_branches = [];
      ...
      if ($project_data['existing_version'] === $version) {
        ...
        elseif (!$release_is_supported) {
          ...
          if (empty($project_data['recommended']) && empty($project_data['also'])) {
            $unsupported_message = 'Your currently installed release is now unsupported, is no longer available for download and no update is available. Uninstalling everything included in this release is strongly recommended!';
          }
          else {
            $unsupported_message = 'Your currently installed release is now unsupported, and is no longer available for download. Uninstalling everything included in this release or upgrading is strongly recommended!';
          }
          $project_data['extra'][] = [
